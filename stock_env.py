...
class stock:
    ...
    def __init__(self, df, init_money=100000, window_size=6):
        ...
        self.buy_min = 0.0002  # 最小买入费率
        self.sell_rate = 0.0003  # 卖出费率
        self.sell_min = 0.0002  # 最小卖出费率
    ...
    def reset(self):
        ...
        self.last_value = self.init_money # 上一天总资产（股票+手里的钱）
    ...
    def step(self, action, show_log=False, my_trick=False):
        ...
        #self.reward = self.maket_value - self.last_value
                
        '''
        这个奖励是股票第二天的涨跌幅，效果不怎么好
    ...
    def draw(self, save_name1, save_name2):
        ...
        plt.plot(close, color='r', lw=2.) #红色
        plt.plot(close, 'v', markersize=8, color='c', label = '卖出指令', markevery = states_sell) #青色
        plt.plot(close, '^', markersize=8, color='k', label = '买入指令', markevery = states_buy) #黑色
        plt.title('总收益率为%f%%'%(invest * 100),fontdict={'size': 24})
        plt.ylabel('股票价格', fontdict={'size': 28}) #坐标名称大小
        plt.xlabel('时间步', fontdict={'size': 28})
        plt.xticks(size = 22) #坐标刻度大小
        plt.yticks(size = 22)
        ...
        plt.rcParams['font.sans-serif']=['SimHei'] #用来正常显示中文标签
        plt.rcParams['axes.unicode_minus']=False #用来正常显示负号
        plt.plot(profit_rate_account, label='DQN')
        plt.plot(profit_rate_stock, label='Stock',linestyle='dashed') #虚线
        plt.ylabel('总收益率', fontdict={'size': 28}) #坐标名称大小
        plt.xlabel('时间步', fontdict={'size': 28})
        plt.xticks(size = 22) #坐标刻度大小
        plt.yticks(size = 22)
        plt.legend(prop={'size': 20})
